...
def xy_biased_sunlight():
    ...
    with open(filename, "w") as f:
        for i in range(n_steps):
            ...
            def neg_y_biased_sunlight():
                orbit_period = 6000  # seconds
                timestep = 10
                n_steps = int(orbit_period / timestep)
                filename = "inputs/neg_y_biased_sun"

                y_bias_strength = 0.40  # 40% bias toward -Y
                ...
                with open(filename, "w") as f:
                    for i in range(n_steps):
                        ...
                        # Apply blend toward -Y direction
                        x *= (1 - y_bias_strength)
                        y = (1 - y_bias_strength) * y + (-1.0) * y_bias_strength  # pulling toward -Y
                        z = 0.0
...
def heavily_neg_y_biased_sunlight():
    orbit_period = 6000  # seconds
    timestep = 10
    n_steps = int(orbit_period / timestep)
    filename = "inputs/heavy_neg_y_biased_sun"

    y_bias_strength = 0.90  # 90% pull toward -Y

    with open(filename, "w") as f:
        for i in range(n_steps):
            phase = (i * timestep) / orbit_period
            angle = 2 * math.pi * phase
            ...
            # Apply 90% pull toward -Y
            x *= (1 - y_bias_strength)
            y = (1 - y_bias_strength) * y + (-1.0) * y_bias_strength
            z = 0.0
...
if __name__ == "__main__":
    ...
    biased_sunlight()
    xy_biased_sunlight()
    neg_y_biased_sunlight()
    heavily_neg_y_biased_sunlight()